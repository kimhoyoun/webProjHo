...
<div id="main_wrap">
	<div>
		<h2 id="main_title">축구 자유게시판 리스트</h2>
	</div>
	<hr>
					...
					<td class="paging" colspan="5">
						<nav aria-label="Page navigation example">
							...
							<c:if test="${pageStart > 1 }">
								<li class="page-item "><a class="page-link" href="<c:url value="/commu_soc/board/SearchList?page=${pageStart - 1 }&field=${param.field }&search=${param.search }"/>">이전</a></li>
							</c:if>	
							<c:forEach var="i" begin="${pageStart }" end="${pageEnd }" step="1">
								<c:choose>
									...
									<c:otherwise>
										<li class="page-item"><a class="page-link" href="<c:url value="/commu_soc/board/SearchList?page=${i }&field=${param.field }&search=${param.search }"/>">${i }</a></li>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							<c:if test="${pageEnd<pageTotal }">
								<li class="page-item"><a class="page-link" href="<c:url value="/commu_soc/board/SearchList?page=${pageEnd + 1 }&field=${param.field }&search=${param.search }"/>">다음</a></li>
							</c:if>	
				            </ul>
			            </nav>
					</td>
