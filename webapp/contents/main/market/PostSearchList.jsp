...
<div id="main_area">
	<div id="postlist_header">
		...
		<div id="postlist_search">
			<form id="category_select" action="PostSearchList" method="post"
				class="input-group mb-3">
				<select id="l_field" name="l_field" class="form-select"
					aria-label="Default select example">
					<option value="list" <c:if test="${l_field == \"전체\"}">selected</c:if>> 전체</option>
					<option value="농구용품" <c:if test="${l_field == \"농구용품\"}">selected</c:if>>농구용품</option>
					<option value="축구용품" <c:if test="${l_field == \"축구용품\"}">selected</c:if>>축구용품</option>
				</select> <select id="s_field" name="s_field" class="form-select"
					aria-label="Default select example">
					<option value="list" <c:if test="${s_field == \"전체\"}">selected</c:if>>전체</option>
					<option value="신발" <c:if test="${s_field == \"신발\"}">selected</c:if>>신발</option>
					<option value="의류" <c:if test="${s_field == \"의류\"}">selected</c:if>>의류</option>
					<option value="기타용품" <c:if test="${s_field == \"기타용품\"}">selected</c:if>>기타용품</option>
				</select> <input type="text" id="search" name="search"
					value="${param.search }" class="form-control" /> <input
					type="submit" value="검색" class="btn btn-outline-secondary" />
			</form>
			<div>
				<button type="button" class="btn btn-outline-secondary" onclick="location.href='PostInsert?page=${nowPage }'">글쓰기</button>
			</div>
		</div>
	</div>
