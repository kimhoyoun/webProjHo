	...
	<div id="postlist_header">
		...
		<div id="postlist_search">
			<form id="category_select" action="PostSearchList" method="post"
				...
				</select> <input type="text" id="search" name="search" class="form-control" />
				<input type="submit" value="검색" class="btn btn-outline-secondary" />
			</form>
			<div>
				<button type="button" class="btn btn-outline-secondary"
					onclick="location.href='PostInsert?page=${nowPage }'">글쓰기</button>
			</div>
		</div>
	</div>
				...
				<div class="row">
							...
							<div class="col"
								...
								<c:choose>
									...
									<c:otherwise>
										<c:set var="res"
											value="/uploadFile/market/${dto.firstImg }" />
									</c:otherwise>
								</c:choose>
			...
			<ul class="pagination justify-content-center"
				...
				<c:if test="${pageStart > 1 }">
					<li class="page-item"><a class="page-link"
						href="<c:url value="/market/PostList?page=${pageStart-1 }"/>">이전</a>
					</li>
				</c:if>
				<c:forEach var="i" begin="${pageStart }" end="${pageEnd }" step="1">
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link"
								href="<c:url value="/market/PostList?page=${i }"/>">${i }</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
