...
public class Lesson_Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceStr = request.getRequestURI().substring(
				(request.getContextPath() + "/lesson/soc/").length()
				);
		...
		if (nonClass.contains(serviceStr)) {
			request.setAttribute("mainUrl", "lesson/soc/" + serviceStr);

		} else {
			// 외워라
			try {
				LessonService service = 
						(LessonService) Class.forName("lesson.soc.service.Lesson" + serviceStr)
