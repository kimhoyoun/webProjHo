...
public class BoardController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceStr = request.getRequestURI().substring(
				(request.getContextPath() + "/mypage/board/").length());
		System.out.println("ServiceStr ==> " + serviceStr);
		
		if(nonClass.contains(serviceStr)) {
			request.setAttribute("mainUrl", "mypage/board/"+serviceStr);
		}else {
			try {
				MypageService serv = (MypageService)Class.forName("mypage.board.service.Board" + serviceStr).newInstance();
