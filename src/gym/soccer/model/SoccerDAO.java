...
public class SoccerDAO {
	...
	public SoccerDAO() {
		...
		sql = "select count(*) from gym_soccer";
		...
		public int searchCnt(String filter, String keyword){
			...
			sql = "SELECT COUNT(*) FROM gym_soccer WHERE address LIKE ? and sname LIKE ? ";
		...
		public int snameSearchCnt(String keyword){
			...
			sql = "select count(*) from gym_soccer where sname like ? ";
		...
		public int filterSearchCnt(String filter){
			...
			sql = "select count(*) from gym_soccer where address like ? ";
		...
		public ArrayList<SoccerDTO> list(int start, int limit){
			ArrayList<SoccerDTO> res = new ArrayList<>();
			
			sql = "select * from gym_soccer order by no desc limit ?, ?";
		...
		public ArrayList<SoccerDTO> search(int start, int limit, String filter, String keyword){
			ArrayList<SoccerDTO> res = new ArrayList<>();
			
			sql = "select * from gym_soccer where address like ? and sname like ? order by no desc limit ?, ?";
		...
		public ArrayList<SoccerDTO> snameSearch(int start, int limit, String keyword){
			ArrayList<SoccerDTO> res = new ArrayList<>();
			
			sql = "select * from gym_soccer where sname like ? order by no desc limit ?, ?";
		...
		public ArrayList<SoccerDTO> filterSearch(int start, int limit, String filter){
			ArrayList<SoccerDTO> res = new ArrayList<>();
			
			sql = "select * from gym_soccer where address like ? order by no desc limit ?, ?";
		...
		public SoccerDTO detail(String id){
			SoccerDTO dto = null;
			
			sql = "select * from gym_soccer where id = ?";
		...
		public void insert(SoccerDTO dto){
			
			sql = "INSERT INTO gym_soccer (id, sname, contents_info, contents_detail, contents_rule,"
	...
	public int modify(SoccerDTO dto){
			int res = 0;
			
			sql = "update gym_soccer set sname = ?, contents_info = ?, contents_detail = ?, contents_rule = ?, contents_refund = ?, "
	...
	public int modifyFile(String id){
		...
		sql = "update gym_soccer set img = null	where id = ? ";
		try {
			ptmt = con.prepareStatement(sql);
			ptmt.setString(1, id);
			res = ptmt.executeUpdate();
		...
		public int delete(SoccerDTO dto){
		int res = 0;
		
		sql = "delete from gym_soccer where id = ?";
		try {
			ptmt = con.prepareStatement(sql);
			ptmt.setString(1, dto.getId());
			res = ptmt.executeUpdate();
