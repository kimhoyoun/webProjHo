...
@WebServlet("/commu_soc/guest/*")
public class Guest_Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceStr = request.getRequestURI().substring(
				(request.getContextPath() + "/commu_soc/guest/").length()
				);
		...
		if (nonClass.contains(serviceStr)) {
			request.setAttribute("mainUrl", "commu_soc/guest/"+serviceStr);

		} else {
			// 외워라
			try {
				GuestService service = 
						(GuestService) Class.forName("commu_soc.guest.service.Guest" + serviceStr)
