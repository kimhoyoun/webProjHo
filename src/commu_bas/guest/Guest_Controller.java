...
public class Guest_Controller extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String serviceStr = request.getRequestURI().substring(
				(request.getContextPath() + "/commu_bas/guest/").length()
				);
		...
		if (nonClass.contains(serviceStr)) {
			request.setAttribute("mainUrl", "commu_bas/guest/"+serviceStr);

		} else {
			// 외워라
			try {
				GuestService service = 
						(GuestService) Class.forName("commu_bas.guest.service.Guest" + serviceStr)
